...
import { LogIn, Plus, LogOut, GraduationCap, ChevronRight, Table as TableIcon, FileText, Printer, QrCode, Circle, X, Check, Trash2 } from "lucide-react";
...
function StudentDetail({ student, progress, onBack, onCreatePaper }: { student: Student, progress: Record<string, Progress>, onBack: () => void, onCreatePaper: () => void }) {
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
      <div className="flex items-end justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 text-[10px] text-slate-400 font-bold bg-white border border-slate-200 px-3 py-2 rounded-lg shadow-sm">
             ...
             <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 bg-green-400 rounded-sm"></div> 2회 연속</div>
             <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 bg-green-500 rounded-sm flex items-center justify-center text-[7px] text-white font-black">★</div> 3회 연속</div>
             <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 bg-red-400 rounded-sm"></div> 1회 틀림</div>
             <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 bg-red-900 rounded-sm flex items-center justify-center text-[7px] text-white">☠</div> 연오답</div>
          </div>
          <button 
          ...
          >
            <FileText className="h-4 w-4" /> Exam Builder
          </button>
        </div>
      </div>
      ...
      <div className="space-y-12">
        {MATH_CONTENT.map(gradeGroup => (
          <div key={gradeGroup.grade} className="space-y-6">
            <h3 className="text-sm font-black text-slate-300 uppercase tracking-[0.2em] border-l-4 border-slate-200 pl-4 mb-8">
              {gradeGroup.grade}
            </h3>
            <div className="space-y-6">
              {gradeGroup.units.map(unit => (
                <section key={unit.title} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm p-6">
                  <div className="flex items-center gap-3 mb-6 pb-2 border-b border-slate-100">
                    <span className="px-2 py-0.5 bg-slate-900 text-white text-[10px] font-bold rounded uppercase tracking-wider">Chapter</span>
                    <h4 className="font-bold text-slate-800">{unit.title}</h4>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {unit.types.map(type => (
                      <div key={type.id} className="p-4 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-slate-50 transition-colors group">
                        <div className="text-[10px] text-slate-500 font-bold mb-3 uppercase tracking-tighter">{type.title}</div>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-[10px] font-bold text-slate-300 group-hover:text-slate-500 transition-colors">
                            {progress[type.id]?.streak || 0} Streak
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      <div className="bg-white p-8 rounded-2xl shadow-2xl border border-slate-200 w-full max-w-xl my-8">
        ...
        {!isSubmitted && (
          <div className="mt-10 p-6 bg-slate-900 rounded-2xl shadow-xl">
            <p className="text-white text-[10px] font-black uppercase tracking-[0.2em] mb-4 opacity-50 text-center">Finalize Grading</p>
            <button 
